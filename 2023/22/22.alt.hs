...
observe :: (Int, Stars) -> Telescope -> S.Set Coord
observe (offset, Stars stars w) = S.fromList . mapMaybe (\(x, y) -> ((x + offset) `mod` w, y) `M.lookup` stars) . S.toList
