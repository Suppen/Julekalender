...
type State = (Int, Int, Direction, Workload)

workCount :: Int
workCount = 1000740

primes :: S.Set Int
primes = S.fromAscList . takeWhile (<workCount) $ 2 : 3 : 5 : primes'
...
rule1 (nextStep, elfNo, direction, workload) =
    if   S.member nextStep primes && ((==1) . length $ nextElf)
    then Just (nextStep, head nextElf, direction, workload)
    else Nothing
    where nextElf = leastWork workload

rule2 :: State -> Maybe State
rule2 (nextStep, elfNo, direction, workload)
    | nextStep `mod` 28 == 0 = Just (nextStep, nextElf, otherDirection, workload)
    | otherwise              = Nothing
    where otherDirection = if direction == CW then CCW else CW
          nextElf = neighbourElf otherDirection elfNo

rule3 :: State -> Maybe State
rule3 (nextStep, elfNo, direction, workload) =
    if   even nextStep && ((==1) . length $ mw) && head mw == neighbour
    then Just (nextStep, nextElf, direction, workload)
    else Nothing
...
rule4 (nextStep, _, direction, workload)
    | nextStep `mod` 7 == 0 = Just (nextStep, 5, direction, workload)
    | otherwise             = Nothing

rule5 :: State -> Maybe State
rule5 (nextStep, elfNo, direction, workload) =
    Just (nextStep, neighbourElf direction elfNo, direction, workload)

step :: State -> State
step (currentStep, elfNo, direction, workload) =
    let newWorkload = work workload elfNo
        nextStep = currentStep+1
    in  fromJust
            . head
            . filter isJust
            $ [rule1, rule2, rule3, rule4, rule5] <*> pure (nextStep, elfNo, direction, newWorkload)
