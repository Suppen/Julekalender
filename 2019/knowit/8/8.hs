...
prizeOps = map (fmap (\f -> \(wheels, prize) -> (wheels, f prize))) [
        ...
        ("GANGEMSD",       (\n -> n * getMsd n)),
        ("DELEMSD",        (\n -> n `div` getMsd n)),
        ("PLUSS1TILPAR",   (\n -> (* sign n) . fromDigits . map (when even ((`mod` 10) . (+1))) . toDigits $ n)),
        ("TREKK1FRAODDE",  (\n -> (* sign n) .fromDigits . map (when odd (subtract 1)) . toDigits $ n))
    ]
        where getMsd     = read . (:[]) . head . filter (/= '-') . show
              toDigits   = map read . map (:[]) . filter (/= '-') . show
