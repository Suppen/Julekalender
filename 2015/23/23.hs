...
cutTrees :: [Int] -> [Int]
cutTrees [] = []
cutTrees trees = length trees : cutTrees remainingTrees
    where minTree = minimum trees
          remainingTrees = filter (/=0) $ map (subtract minTree) trees

main = putStrLn $ intercalate "," $ map show $ cutTrees trees
